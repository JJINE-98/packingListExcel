...
export function PdfPreview({ pageUrls }: Props) {
  if (!pageUrls.length) {
    return <div className="flex h-full min-h-[520px] items-center justify-center rounded-xl border border-dashed bg-slate-50 text-sm text-slate-500">패킹리스트를 선택하면 PDF가 표시됩니다.</div>;
  }
  return (
    <div className="h-full min-h-[520px] space-y-3 overflow-auto rounded-xl border bg-slate-200 p-3">
