...
4. PACKING LIST 상품 행을 열별 상대 좌표로 잘라 5배 확대하고 대비 보정
5. Customer와 Product는 문자 OCR, Size/KG·수량·중량은 숫자 전용 OCR로 재인식
6. 셀 단위 인식 결과를 구조화 마커로 저장한 뒤 라벨 기반 원문 결과보다 우선 적용
7. Destination과 Customer는 줄 단위로 제한하고 `KOREA` 국가명을 정규화
8. 수량 합계 및 `Size/KG × Total Quantity`로 누락값 보조 계산
9. `IOcrProvider` 인터페이스로 CLOVA, Google Vision, Upstage, OpenAI JSON 정규화 모듈 교체 가능
...
- 사용자가 브라우저에서 기존 `.xlsx` 파일을 선택할 수 있다.
- 첫 번째 시트가 `출고요청서`인지 확인한 뒤 해당 시트만 수정한다.
- `B/L No.` 헤더, 합계 수식 행, PDF와 일치하는 AWB 블록을 동적으로 탐색한다.
- 데이터 영역에 빈 행이 있으면 재사용한다.
- 빈 행이 없으면 합계 행 위에 신규 행을 삽입하고 기존 데이터 행의 스타일을 복제한다.
- 빈 셀의 스타일 스텁까지 읽고 대상 행의 누락된 셀 스타일을 인접 행에서 보완한다.
- 관리 Excel은 통합문서를 새로 직렬화하지 않고 원본 XLSX ZIP에서 첫 번째 시트 XML만 수정한다.
- `xl/styles.xml`, 테마, 공유문자열, 인쇄설정과 나머지 시트 파일은 원본 바이트를 유지한다.
- 행과 셀 주소를 XML DOM으로 각각 이동해 값 없는 합계 셀의 배경색·테두리도 전체 범위에서 유지한다.
- 합계 행과 하단 영역을 한 행 아래로 이동하고 `SUM` 범위를 신규 행까지 확장한다.
- 다른 시트의 값과 수식은 수정하지 않는다.
- AWB 블록을 찾지 못하면 임의 열에 쓰지 않고 사용자에게 오류를 알린다.
